...
and zona_dokumen='$val->nama_zona' and status='belum_diproses' order by id_draft_permohonan DESC");
                                            } else if($this->session->userdata('tahun')=="2024"){
                                                $get_pengajuan_baru = $this->db->query("select * from tbl_draft 
where left(tgl_input,4)=".$this->session->userdata('tahun')." 
and zona_dokumen='$val->nama_zona'  order by id_draft_permohonan DESC");
                                            }

                                            if(($get_pengajuan_baru->num_rows()>0 && $this->session->userdata('level')=='kasub_perancang')
                                                or($get_pengajuan_baru->num_rows()>0 && $this->session->userdata('level')=='superadmin')){
                                                ?>
                                                <label for="label" class="label" style="background-color: red">
                                                    <?= $get_pengajuan_baru->num_rows()?>
                                                </label>
                                                <?php
                                            } else if($get_pengajuan_baru->num_rows()<=0){
                                                ?>
...
where left(tgl_input,4)=".$this->session->userdata('tahun')." 
and zona_dokumen='$val->nama_zona' and status='sedang_diproses' and id_perancang='$perancang_id' order by id_berita DESC");

                                            } else {
                                                $get_sedang_diproses = $this->db->query("select * from tbl_berita 
where left(tgl_input,4)=".$this->session->userdata('tahun')." 
and zona_dokumen='$val->nama_zona' and status='sedang_diproses' order by id_berita DESC");
                                            }



                                            if(($get_sedang_diproses->num_rows()>0 && $this->session->userdata('level')=='kasub_perancang')
                                                or ($get_sedang_diproses->num_rows()>0 && $this->session->userdata('level')=='superadmin')
                                                or ($get_sedang_diproses->num_rows()>0 && $this->session->userdata('level')=='pemda')
                                                or ($get_sedang_diproses->num_rows()>0 && $this->session->userdata('level')=='perancang')){
                                                ?>
                                                <label for="label" class="label" style="background-color: red">
                                                    <?= $get_sedang_diproses->num_rows()?>
                                                </label>
                                                <?php
                                            } else if($get_sedang_diproses->num_rows()<=0){
                                                ?>
